...
const activeCaseId = computed(() => String(route.params.caseId || localStorage.getItem('active_case_id') || ''));
...
watch(activeCaseId, async (caseId) => {
  if (!caseId) return;
...
function updateMentionSuggestions() {
  ...
  mentionSuggestions.value = evidence.value
    .filter((item) => evidenceTitle(item).includes(keyword) || item.evidence_id.includes(keyword))
    .slice(0, 8);
}

function insertMention(item: EvidenceRecord) {
  followup.value = followup.value.replace(/@([^\s@，。；,;]*)$/, `@[${evidenceTitle(item)}](${item.evidence_id}) `);
  mentionSuggestions.value = [];
}

function evidenceTitle(item: EvidenceRecord) {
  return item.title || item.name || item.evidence_id;
}
