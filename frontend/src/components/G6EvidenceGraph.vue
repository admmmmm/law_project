...
async function renderGraph(seq: number) {
  ...
  console.log('[G6EvidenceGraph:beforeRender]', {
    component: 'G6EvidenceGraph',
    activeLayer: props.layer,
    layoutMode: props.layoutMode,
    layoutType: layout.type,
    nodes: data.nodes.length,
    edges: data.edges.length,
    firstInputNodes: data.nodes.slice(0, 5).map((node: any) => ({
      id: node.id,
      x: node.style?.x,
      y: node.style?.y,
  ...
  if (lastPerfSignature !== renderSignature) {
...
function buildG6Plugins(profile: InteractionProfile, nodeCount: number, edgeCount: number) {
  const enableMiniMap =
    profile === 'full' ||
    (props.layer === 'document' &&
      nodeCount <= 300 &&
      edgeCount <= 600 &&
      typeof window !== 'undefined' &&
      window.localStorage?.getItem('g6_minimap') === 'true');
