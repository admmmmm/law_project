...
def _is_deepseek_provider(model_name: Optional[str], base_url: Optional[str]) -> bool:
    model_name = (model_name or "").lower()
    base_url = (base_url or "").lower()
    return model_name.startswith("deepseek") or "deepseek.com" in base_url


def _get_provider_api_key(model_name: Optional[str], base_url: Optional[str]) -> Optional[str]:
    if _is_deepseek_provider(model_name, base_url):
...
class CacheOpenAI(BaseLLM):
    ...
    def __init__(self, cache_dir, global_config, cache_filename: str = None,
        ...
        self.is_deepseek = _is_deepseek_provider(self.llm_name, self.llm_base_url)
        ...
        if self.global_config.azure_endpoint is None:
            ...
            api_key = _get_provider_api_key(self.llm_name, self.llm_base_url)
    ...
    def infer(
        ...
        **kwargs
        ...
        if self.is_deepseek:
            # DeepSeek exposes an OpenAI-compatible Chat Completions API, but does not accept
            # OpenAI-only sampling knobs such as `seed`.
