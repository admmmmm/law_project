...
def _extract_ner_from_response(real_response):
    payload = _extract_json_object(real_response)
    entities = payload.get("named_entities", []) if isinstance(payload, dict) else []
    return entities if isinstance(entities, list) else []


def _extract_typed_entities(real_response):
    payload = _extract_json_object(real_response)
    entities = payload.get("entities", []) if isinstance(payload, dict) else []
    return [item for item in entities if isinstance(item, dict) and str(item.get("text", "")).strip()]
...
def _normalize_triple_payload(triples: list[Any]) -> tuple[list[list[str]], dict[str, dict[str, Any]]]:
    plain: list[list[str]] = []
    typed_by_key: dict[str, dict[str, Any]] = {}
    for item in triples:
        if isinstance(item, dict):
            ...
            plain.append(triple)
            typed_by_key[_triple_key(subject, relation, obj)] = item
        elif isinstance(item, (list, tuple)) and len(item) >= 3:
            triple = [str(item[0]).strip(), str(item[1]).strip(), str(item[2]).strip()]
            if all(triple):
                plain.append(triple)
    return plain, typed_by_key
...
class OpenIE:
    ...
    def ner(self, chunk_key: str, passage: str) -> NerRawOutput:
        ...
        try:
            ...
            metadata["typed_entities"] = _extract_typed_entities(real_response)
    ...
    def triple_extraction(self, chunk_key: str, passage: str, named_entities: List[str]) -> TripleRawOutput:
        def _extract_triples_from_response(real_response):
            ...
            typed_triples = payload.get("typed_triples", []) if isinstance(payload, dict) else []
            if isinstance(triples, list) and isinstance(typed_triples, list):
                triples = [*triples, *typed_triples]
            elif isinstance(typed_triples, list):
        ...
        try:
            ...
            plain_triples, typed_by_key = _normalize_triple_payload(extracted_triples)
            triplets = filter_invalid_triples(triples=plain_triples)
            metadata["typed_triples"] = [typed_by_key[_triple_key(*triple)] for triple in triplets if _triple_key(*triple) in typed_by_key]
